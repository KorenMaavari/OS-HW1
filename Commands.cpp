             ...
             (id_s[0] == '-' && std::all_of(id_s.begin()+1, id_s.end(), ::isdigit))) )
        {
            ...
            for (auto &job : *SmallShell::getInstance().getJobsList())
            {
                if (job.getJobId() == id)
                {
                    ...
                    if (kill(job.getJobId(),signum) == -1)
                        perror("smash error: kill failed");
                    else
                        std::cout << "signal number " << signum << " was sent to pid " << job.getJobId() << std::endl;
                    if (signum == 20)
...
ExternalCommand::ExternalCommand(const char *cmd_line)
    ...
    if(m_is_bg){
        char *tmp = const_cast<char *>(cmd_line);
    ...
    if( pid < 0 ) {
        ...
        if(m_cmd_line.find('*') != std::string::npos || m_cmd_line.find('?') != std::string::npos){
            ...
            char* arg_arr[] ={const_cast<char *>(bash.c_str()), const_cast<char *>(c.c_str()),
                              const_cast<char *>(m_cmd_line.c_str()), NULL};
...
void ChmodCommand::execute() {
    if(numOfArgs !=3){
        ...
        if(chmod(path.c_str(), mode) == -1){
            std::cerr << "smash error: chmod failed" << std::endl;
        }
...
void ForegroundCommand::execute() {
    ...
    if (numOfArgs == 1 && SmallShell::getInstance().getJobsList()->getSize() == 0) { // אין אינפוט והליסט ריק
    ...
    if(numOfArgs == 2){
        ...
        if (!SmallShell::getInstance().jobIdExist(theJobID)) { // יש אינפוט והג'וב לא קיים
            std::cerr << "smash error: fg: job-id "<< theJobID << " does not exist" << std::endl;
        }

    }

    if (numOfArgs == 1) { // אין אינפוט
        int max_job_id = SmallShell::getInstance().getJobsList()->getMaxJobID();
        theJobID = max_job_id;
    }
    try{
    ...
    if(new_path == "-"){
        if(!m_previous_path.empty()){
            ...
            if(chdir(m_current_path.c_str())!=0){
                std::cerr << "smash error: could not change directory" << std::endl;
            }
        }
        else {
...
m_pid(pid), m_cmd_line(cmd_line), m_status(bg){}
...
void JobsList::removeFinishedJobs() {
    ...
    if(m_job_entry_list->empty()){
        ...
        m_max_job_id =  m_job_entry_list->end()->getJobId();
    }

}

//bool JobsList::compareById(const JobsList::JobEntry &job1, const JobsList::JobEntry &job2) {
...
void JobsList::addJob(Command *cmd, bool isStopped) {
    JobEntry job(m_max_job_id+1, cmd->pid, cmd->cmd_line.c_str(), JobEntry::Status::bg);
...
void PipeCommand::execute() {
    ...
    if (fork() == 0) { // first child
        ...
        std::cerr << "smash error:> failed fork" << std::endl;
    }
